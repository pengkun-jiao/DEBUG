...
def hed(tenInput, device):
    ...
    edges = netNetwork(tenInput.view(1, 3, intHeight, intWidth))
...
#             edgs = self.network(batch)

if __name__ == "__main__":
    img_path = 'output/n02106662_2101-2.png'
    out = 'output/edge/n02106662_2101-2.jpg'

    tenInput = torch.FloatTensor(np.ascontiguousarray(
        np.array(PIL.Image.open(img_path))[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) * (1.0 / 255.0)))

    tenOutput = hed(tenInput).cpu()

    PIL.Image.fromarray((tenOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(np.uint8)).save(out)