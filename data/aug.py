...
augmentation = (
                ...
                [ToEdges2D(sigma=args.edges_sigma, dil=args.edges_dil),
                 transforms.GaussianBlur(args.canny_blur_rad, 0.15), StretchValues()]
            )




img_path = 'output/056_0011.jpg'
with Image.open(img_path, 'r') as image:
    img_tensor = transform(image)
    # input_tensor = torch.unsqueeze(img_tensor, dim=0) #[C, H, W] -> [N, C, H, W]
    toEdges = ToEdges()
    img = toEdges(img_tensor)
    img = transforms.ToPILImage()(img)
    path = 'output/edge'
    if not os.path.exists(path):
        os.makedirs(path)
    path += 'n02503517_3808.jpg'
    img.save(path)
